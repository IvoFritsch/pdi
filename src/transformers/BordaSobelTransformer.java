...
public class BordaSobelTransformer extends Transformer{
    ...
    public Imagem transform(){
        ...
        img1.percorrePixelsImagem(p -> {
            ...
            int resultado = (int)Math.sqrt(Math.pow(p.getEscalaCinza(), 2) + 
                                            Math.pow(img2.getPixel(p.x, p.y).getEscalaCinza(), 2));
            resultado = resultado > getInputValue("threshold") ? 255 : 0;
